<?php
class ControllerExtensionModuleBossBulkOrder extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('extension/module/boss_bulk_order');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('module_boss_bulk_order', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/boss_bulk_order', 'user_token=' . $this->session->data['user_token'], true)
		);

		$data['action'] = $this->url->link('extension/module/boss_bulk_order', 'user_token=' . $this->session->data['user_token'], true);

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);


		$data['module_boss_bulk_order'] = array();

		if (isset($this->request->post['module_boss_bulk_order'])) {
			$data['module_boss_bulk_order'] = $this->request->post['module_boss_bulk_order'];
		} else {
			$data['module_boss_bulk_order'] = $this->config->get('module_boss_bulk_order');
		}

		//echo'<pre>';print_r($data['module_boss_bulk_order']);echo'</pre>';
		if (!empty($data['module_boss_bulk_order'])) {
			$data['status'] = $data['module_boss_bulk_order']['status'];
			//$data['search_price'] = $data['module_boss_bulk_order']['search_price'];
			$data['num_products'] = $data['module_boss_bulk_order']['num_products'];
			$data['num_record'] = $data['module_boss_bulk_order']['num_record'];
			$data['search_product'] = $data['module_boss_bulk_order']['search_product'];
			$data['search_category'] = $data['module_boss_bulk_order']['search_category'];
			$data['search_model'] = $data['module_boss_bulk_order']['search_model'];
			$data['search_tags'] = $data['module_boss_bulk_order']['search_tags'];
			$data['search_price'] = $data['module_boss_bulk_order']['search_price'];
			$data['image_width'] = $data['module_boss_bulk_order']['image_width'];
			$data['image_height'] = $data['module_boss_bulk_order']['image_height'];
			$data['price_min'] = $data['module_boss_bulk_order']['price_min'];
			$data['price_max'] = $data['module_boss_bulk_order']['price_max'];
			$bulk_option = isset($data['module_boss_bulk_order']['option'])?$data['module_boss_bulk_order']['option']:array();
		}else{
			$data['status'] = 0;
		...
		$this->response->setOutput($this->load->view('extension/module/boss_bulk_order', $data));
	}

	private function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/boss_bulk_order')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		if (isset($this->request->post['module_boss_bulk_order']) && !empty($this->request->post['module_boss_bulk_order'])) {
			$boss_bulk_order = $this->request->post['module_boss_bulk_order'];
