	...
	public function index() {
	   ...
	   if (isset($this->request->get['page'])) {
		...
		if (isset($this->request->get['limit'])) {
			...
			$limit = $this->config->get('module_bossblog_limit');
		}
		...
		if ($blogcategory_info) {
			...
			if ($blogcategory_info['image']) {
				$data['thumb'] = $this->model_tool_image->resize($blogcategory_info['image'], $this->config->get('module_bossblog_image_article_width'), $this->config->get('module_bossblog_image_article_height'));
			} else {
				$data['thumb'] = '';
			}
			...
			foreach ($results as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], $this->config->get('module_bossblog_image_category_width'), $this->config->get('module_bossblog_image_category_height'));
				} else {
					$image = false;
				}
		   ...
		   $url = '';
			...
			$limits = array_unique(array($this->config->get('module_bossblog_limit'), 10, 15, 20, 30));
			...
			$this->response->setOutput($this->load->view('bossblog/blogcategory', $data));
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('theme_' . $this->config->get('config_theme') . '_directory') . '/template/error/not_found')) {
				$this->template = $this->config->get('theme_' . $this->config->get('config_theme') . '_directory') . '/template/error/not_found';
			} else {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('theme_' . $this->config->get('config_theme') . '_directory') . '/template/error/not_found')) {
				$this->response->setOutput($this->load->view($this->config->get('theme_' . $this->config->get('config_theme') . '_directory') . '/template/error/not_found', $data));
			} else {
