...
class ControllerDesignBossLayout extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('design/boss_layout', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		foreach ($layouts as $layout) {
			...
			$data['layouts'][] = array(
				...
				'name'      => $layout['name'],
				'edit'      => $this->url->link('design/boss_layout/edit', 'user_token=' . $this->session->data['user_token'] . '&layout_id=' . $layout['layout_id'] , 'SSL')
			);
		}
		...
		$data['module_manager'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'], 'SSL');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('design/boss_layout', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL')
		);
		...
		$data['action'] = $this->url->link('design/boss_layout/edit', 'user_token=' . $this->session->data['user_token'] . '&layout_id=' .$data['layout_id']. $url, 'SSL');

		$data['cancel'] = $this->url->link('common/dashboard', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL');
		
		$data['add_module'] = $this->url->link('design/boss_layout/modules', 'user_token=' . $this->session->data['user_token'], 'SSL');
		...
		foreach ($extensions as $code) {
			...
			if(empty($modules)){
				$module_data[] = array(
					...
					'code' => $code,
					'edit'      => $this->url->link('extension/module/' . $code . '', 'user_token=' . $this->session->data['user_token'], 'SSL'),
					'status' => false
				);
			}else{
				foreach ($modules as $module) {
						$setting_module = json_decode($module['setting'], true);
						$module_data[] = array(
							'name' => $this->language->get('heading_title') . ' &gt; ' . $module['name'],
							'code' => $code . '.' .  $module['module_id'],
							'edit'      => $this->url->link('extension/module/' . $code, 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $module['module_id'], 'SSL'),
		...
		if (isset($this->request->post['layout_module'])) {
			...
			foreach ($layout_modules as $layout_module) {
					...
					if(!isset($part[0])){
						$href = $this->url->link('extension/module/' . $layout_module['code'], 'user_token=' . $this->session->data['user_token'], 'SSL');
					}elseif (isset($part[0]) && !isset($part[1])) {						
						$href = $this->url->link('extension/module/' . $part[0], 'user_token=' . $this->session->data['user_token'], 'SSL');
					}else{						
						$href = $this->url->link('extension/module/' . $part[0], 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $part[1], 'SSL');
					}
		...
		$this->response->setOutput($this->load->view('design/bossthemes/layout_form', $data));
	}
	...
	public function module_list(){
		...
		foreach ($extensions as $code) {
			...
			if(empty($modules)){
				$module_data[] = array(
					'name' => $this->language->get('heading_title'),
					'code' => $code,
					'edit'      => $this->url->link('module/' . $code . '', 'user_token=' . $this->session->data['user_token'], 'SSL'),
				);
			}else{
				foreach ($modules as $module) {
						$module_data[] = array(
							'name' => $this->language->get('heading_title') . ' &gt; ' . $module['name'],
							'code' => $code . '.' .  $module['module_id'],
							'edit'      => $this->url->link('module/' . $code, 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $module['module_id'], 'SSL')
						);
				}
			}
			if ($this->config->has($code . '_status') || $module_data) {
		...
		$this->response->setOutput($this->load->view('design/bossthemes/module_list', $data));
	...
	public function modules() {		
		...
		$this->response->setOutput($this->load->view('boss_layout/modules', $data));
	}
