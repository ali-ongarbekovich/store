...
class ControllerExtensionModuleBossMenucategory extends Controller {
	...
	public function index() {
		$this->language->load('extension/module/boss_menucategory');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('boss_menucategory', $this->request->post);
			} else {
		...
		$data['entry_sub_category_width'] = $this->language->get('entry_sub_category_width');
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/boss_menucategory', 'user_token=' . $this->session->data['user_token'], 'SSL'),
			'separator' => ' :: '
		);
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/boss_menucategory', 'user_token=' . $this->session->data['user_token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/module/boss_menucategory', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		foreach ($menus as $key => $menu) {
			if (!empty($menu['icon']) && file_exists(DIR_IMAGE . $menu['icon'])) {
		...
		$this->response->setOutput($this->load->view('extension/module/boss_menucategory', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/boss_menucategory')) {
