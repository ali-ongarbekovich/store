	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('boss_testimonial', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/boss_testimonial', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/boss_testimonial', 'user_token=' . $this->session->data['user_token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/module/boss_testimonial', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}


		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', 'SSL');

		$data['edit_testimonials_path'] = $this->url->link('catalog/boss_testimonial', 'user_token=' . $this->session->data['user_token'], 'SSL');
		...
		//print_r($this->config->get('module_bossblog_limit'));
...
//echo'<pre>';print_r($setting);echo'</pre>';die();
		$this->response->setOutput($this->load->view('extension/module/boss_testimonial', $data));
	}

	private function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/boss_testimonial')) {
